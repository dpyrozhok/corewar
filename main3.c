...
/*   Updated: 2018/09/23 20:18:36 by dpyrozho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int num_command)
{
	...
	while (i < 3 && (z = OP(num_command).args[i]) != 0)
	{
		...
		if ((z == 1 || z == 3 || z == 5 || z == 7) &&
(ft_reg(line, inf, (i == 3) ? 0 : OP(num_command).args[i])))
			continue;
		if ((z == 2 || z == 3 || z == 6 || z == 7) &&
(ft_dir(line, inf, (i == 3) ? 0 : OP(num_command).args[i])))
			continue;
		if ((z == 4 || z == 5 || z == 6 || z == 7) &&
(ft_ind(line, inf, (i == 3) ? 0 : OP(num_command).args[i])))
			continue;
		else
			return (0);
	}
